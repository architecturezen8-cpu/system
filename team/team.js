// ==========================================
// TEAM.JS - V31.0 RTDB + CACHE + GITHUB CDN DUAL-UPLOAD
...
const CONFIG = {
    ...
    // GitHub CDN (jsDelivr) - for public Spector site images
    GITHUB: {
        token: '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',
        owner: 'architecturezen8-cpu',
        repo: 'web-assets',
        path: 'battle-of-the-golds/players'
    },
...
let isLoadingTeams = false; // 🆕 Race condition fix
...
document.addEventListener('DOMContentLoaded', () => {
    // ⚡ INIT AUTH SYSTEM - Team Manager role ekata check karanna
    ...
    // ⚡ WAIT FOR AUTH APPROVAL before initializing Team Manager
    document.addEventListener('auth-approved', (e) => {
        const { user, userData } = e.detail || {};
        console.log('✅ Auth approved for Team Manager!', userData);

        // Auto-fill and show user profile in header
        updateHeaderProfile(user);

        // Initialize Firebase only after auth is approved
        initFirebase();
    });

    // ✅ LISTEN FOR ACCESS REVOCATION - Force cleanup if role removed
    document.addEventListener('auth-revoked', (e) => {
        const { reason, isRoleRevoke } = e.detail || {};

        console.warn('🚫 Team access revoked:', reason, isRoleRevoke ? '(role revoke)' : '(status deny)');

        // Stop Firebase connections
        if (db && matchId) {
            try {
        ...
        if (db) {
            try {
                ...
                db.ref(`matches/${matchId}/command`).off();
                db.ref('data_version/teams').off();
            } catch (err) { }
        }
        ...
        if (isRoleRevoke) {
            showErrorPopup('Access Removed', 'Your Team Manager access has been removed. You can request it again from the administrator.');
        } else {
            showErrorPopup('Access Revoked', reason || 'Contact administrator for more information.');
        }

        // Auth.js will automatically show No Access / Denied overlay
        // Listener stays alive for auto-re-approve when admin adds role back
    });

    // Note: loadTeams() is NOT called here — it's called after Firebase init + connection test
});
...
function updateHeaderProfile(user) {
    // Accept user from auth event or get from auth module
    ...
    try {
        ...
        const emailSpan = document.getElementById('headerUserEmail');
...
async function uploadImageToImgBB(file) {
    return new Promise(async (resolve, reject) => {
        try {
            const resizedBase64 = await cropImageToSquare(file, 600);
...
function cropImageToSquare(file, size = 600) {
...
// ==========================================
// GITHUB CDN UPLOAD (jsDelivr)
// Dual-upload: ImgBB (internal) + GitHub (public Spector)
// ==========================================
async function uploadImageToGitHub(file, folder) {
    const GITHUB_API = 'https://api.github.com';
    const cfg = CONFIG.GITHUB;
    if (!cfg.token) {
        console.warn('⚠️ GitHub token not configured, skipping CDN upload');
        return null;
    }
    ...
    try {
        ...
        const githubPath = folder ? `${cfg.path}/${folder}/${fileName}` : `${cfg.path}/${fileName}`;
        ...
        const url = `${GITHUB_API}/repos/${cfg.owner}/${cfg.repo}/contents/${githubPath}`;
        let existingSha = null;
        try {
            const checkRes = await fetch(url, {
                headers: { 'Authorization': `token ${cfg.token}`, 'User-Agent': 'BOTG-TeamManager' }
            });
            if (checkRes.ok) {
        ...
        const body = {
            message: `Upload ${fileName} via BOTG Team Manager`,
            content: base64,
            branch: 'main'
        };
        ...
        const cdnUrl = `https://cdn.jsdelivr.net/gh/${cfg.owner}/${cfg.repo}/${githubPath}`;
        console.log('✅ Image uploaded to GitHub CDN:', cdnUrl);
        return cdnUrl;
    } catch (error) {
        console.error('GitHub CDN upload error:', error);
        return null; // Non-critical - ImgBB is primary
    }
}
...
async function testConnection() {
    ...
    try {
        ...
        // ✅ ALWAYS RESET BUTTON (Success or Fail)
        if (btnTest) {
...
// ==========================================
// 🆕 LOAD TEAMS (RTDB + Cache System) - NO BUG VERSION
// ==========================================
async function loadTeams(forceRefresh = false) {
    // Race condition protection - ekath ekka run wenne na
    if (isLoadingTeams) return;
    isLoadingTeams = true;

    const btnRefresh = document.getElementById('btnRefresh');
    const btnRefreshConn = document.getElementById('btnRefreshConn');
    ...
    if (btnRefreshConn) {
        btnRefreshConn.innerHTML = '<span class="spinner-sm"></span> Refreshing...';
        btnRefreshConn.disabled = true;
    }
    ...
    try {
        ...
        teams = Object.entries(teamsData).map(([id, team]) => {
            ...
            return { id, ...team, players: teamPlayers };
        });
        ...
        document.getElementById('connPing').textContent = usedCache ? '0 ms (cache)' : pingTime + ' ms';
        ...
        if (cached && cached.teams) {
            ...
            teams = Object.entries(cached.teams).map(([id, team]) => ({
                id, ...team, players: playersArray.filter(p => p.team_id === id)
            }));
            renderTeams();
        } else {
    ...
    // ✅ ALWAYS HARDCODE RESET - Original text save karanne ne, direct danna eka
    if (btnRefresh) {
        btnRefresh.innerHTML = '🔄 Refresh';
        btnRefresh.disabled = false;
    }
    if (btnRefreshConn) {
        btnRefreshConn.innerHTML = '♻️ Refresh Data';
        btnRefreshConn.disabled = false;
    }

    isLoadingTeams = false; // Unlock
}
...
function openTeamModal(teamId) {
    ...
    const cdnInput = document.getElementById('inputLogoCdnUrl');
    ...
    if (teamId) {
        ...
        if (team) {
            ...
            if (logoSrc) {
                ...
                const cdnInput = document.getElementById('inputLogoCdnUrl');
                if (cdnInput) cdnInput.value = team.logo_cdn_url || '';
...
function addPlayerRow(player) {
    ...
    const playerCdnUrl = player && player.photo_cdn_url ? player.photo_cdn_url : '';
    ...
    row.innerHTML =
        ...
        '<input type="hidden" data-field="photoCdnUrl" value="' + escapeAttr(playerCdnUrl) + '">' +
...
async function handlePlayerPhotoUpload(input, rowNum) {
    ...
    try {
        showLoading('Uploading player photo...', 'Uploading to ImgBB + GitHub CDN');

        // 1. Upload to ImgBB (internal use: Score Bar, Monitor, OBS)
        const imgbbUrl = await uploadImageToImgBB(file);

        // 2. Upload to GitHub jsDelivr CDN (public: Spector)
        const cdnUrl = await uploadImageToGitHub(file, 'players');

        const row = document.getElementById('player-row-' + rowNum);
        if (!row) return;

        const hiddenInput = row.querySelector('[data-field="photoUrl"]');
        const cdnInput = row.querySelector('[data-field="photoCdnUrl"]');
        ...
        if (cdnInput) cdnInput.value = cdnUrl || '';
        ...
        hideLoading();
        showSuccessPopup('Photo Uploaded!', cdnUrl ? 'Saved to ImgBB + GitHub CDN' : 'Saved to ImgBB (CDN failed)');
    } catch (error) {
        console.error(error);
        hideLoading();
...
async function handleLogoUpload(input) {
    ...
    showLoading('Uploading logo...', 'Uploading to ImgBB + GitHub CDN');

    try {
        // 1. Upload to ImgBB (internal use)
        const imgbbUrl = await uploadImageToImgBB(file);

        // 2. Upload to GitHub jsDelivr CDN (public: Spector)
        const cdnUrl = await uploadImageToGitHub(file, 'logos');

        document.getElementById('inputLogoUrl').value = imgbbUrl;
        // Store CDN URL in hidden field
        const cdnInput = document.getElementById('inputLogoCdnUrl');
        if (cdnInput) cdnInput.value = cdnUrl || '';
        ...
        hideLoading();
        showSuccessPopup('Logo Uploaded!', cdnUrl ? 'Saved to ImgBB + GitHub CDN' : 'Saved to ImgBB (CDN failed)');
    } catch (error) {
        console.error(error);
        hideLoading();
        showErrorPopup('Upload Failed', 'Could not upload to ImgBB. Please try again.');
...
async function saveTeam() {
    ...
    for (let i = 0; i < playerRows.length; i++) {
        ...
        if (nameInput && nameInput.value.trim()) {
            ...
            players.push({
                ...
                photo_cdn_url: cdnInput && cdnInput.value ? cdnInput.value : null,
    ...
    try {
        ...
        const logoCdnUrl = document.getElementById('inputLogoCdnUrl')?.value || '';
        ...
        if (players.length > 0) {
            ...
            players.forEach(p => {
                ...
                updates['players/' + newPlayerKey] = {
                    ...
                    photo_cdn_url: p.photo_cdn_url || null,
...
function showToast(message, type = 'success') {
    // Use existing success/error popups as toast fallback
    if (type === 'error') {
        showErrorPopup('Access Revoked', message);
    } else {
        showSuccessPopup('Notice', message);
    }
}
